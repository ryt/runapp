...
> When using runapp, a directory named `~/.runapp` will be created automatically in your home directory to store the process ids for each app/process that you run. The pids will get deleted automatically whenever the process is stopped. You can use the pid number to debug any issues you may run into when deploying apps.
